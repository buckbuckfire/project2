...
$("#search-youtube").on("click", function() {
  ...
  $.get("/api/" + searchedVideo, function(data) {
    ...
    if (!data) {
      ...
      // otherwise
      // append the character name
      $("#video-details").append("<h2>" + data.name + "</h2>");
      // the role
      $("#video-details").append("<h3>Role: " + data.role + "</h3>");
      // the age
      $("#video-details").append("<h3>Age: " + data.age + "</h3>");
      // and the force points
      $("#video-details").append("<h3>Force Points: " + data.forcePoints + "</h3>");
    }
  });
});
